...
public sealed class DeleteCategoryUseCase : IDeleteCategoryUseCase
{
    ...
    public async Task ExecuteAsync(DeleteCategoryInput input, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await _categoryRepository.DeleteAsync(input.Id, cancellationToken);
            await _unitOfWork.CommitAsync(cancellationToken);
        }
        catch (Exception ex)
        {
            await _unitOfWork.RollbackAsync(cancellationToken);
            _logger.LogError(ex, "Error while deleting category {CategoryId}", input.Id);
            throw;
        }
    }
}
